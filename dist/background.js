...
function saveStateToLocalStorage() {
    var state = {
        uploadedJson, seleniumJson, stop, playback, autoPublish, pauseMode, demoMode, curDocInfo, pageEntryArray
    }
...
chrome.runtime.onMessage.addListener(
    (request, sender, sendResponse) => {
        if (request.message === "start_clicked_in_popup") {
        ...
        } else if (request.message === "save_clicked_from_popup" || request.message === "FROM_RECORD_SAVE") {
            saveJsonToLocalStorage(request.data);
        } else if (request.message === "FROM_RECORD_COMPLETE" || request.message === "stop_clicked_in_popup" ||
            ...
            sendResponse({ stop: stop, playback: playback, playbackJson: uploadedJson, curDocInfo });
...
function saveJsonToLocalStorage(data) {
    ...
    if(data.stop) {
        ...
        if(data.viewer == "In"){
            sendMessageToProxy("DISGARD_RECORD_OR_PLAYBACK", { code: "disregardRecordOrPlayback();" });
        }        
    }
...
function analyzePageInitInfo(request, sendResponse) {
    ...
    if (playback && stop) {
        if (isPlaybackInSameTransaction() || !uploadedJson.meta.transRefId) {
            ...
            sendMessageToProxy("playback_in_wrong_transaction", { code: "msgBox.printErrorMessage(9)" });
            playback = false;
        }
...
function isRecordingInSameTransaction() {
    return curDocInfo && seleniumJson.meta && parseInt(seleniumJson.meta.companyId) === parseInt(curDocInfo.companyId)
        && seleniumJson.meta.transactionName === curDocInfo.transactionName;
}

function isPlaybackInSameTransaction() {
    return curDocInfo && uploadedJson.meta && (uploadedJson.meta.transactionName === curDocInfo.transactionName || uploadedJson.meta.transRefId === curDocInfo.transRefId);
}
                ...
                if (curDocInfo.viewer === "responsive") {
                    ...
                    if (recipientList.length) {
                        for (var i = 0; i < recipientList.length; i++) {
                            if (recipientList[i].level === 1 && recipientList.email !== "system-workflow") {
...
function analyseUploadedFile(request) {
    if (request.message === "playback_called_from_popup_Current_Page_localStorage") {
...
function dataReadyAndStartPlayback(request) {
    ...
    if (request.message.includes("playback_called_from_popup_Current_Page") || request.message.includes("SKIP_LOGIN")) {
...
function writeSeleniumActions(request) {
    console.log("*Background* - Received message to get selenium actions from current page.");
...
AttachmentUtil.uploadAttachmentUsingAPI_old = function () {
    ...
    attachmentList.forEach(attachment => {
        ...
        const promise = fetch(link, {
            ...
            .then(text => {
                ...
                if (!success)
                    console.error(`*Background* - Failed to upload ${attachment.displayName}; Attachment Type: ${attachment.isRaw ? 'File' : 'Document'}.`);
                return {
                    success,
    ...
    return Promise.all(promiseList)
        .then(resultList => {
            ...
            console.log(`*Background* - Attachment Uploaded, ${successNumber} succeeded, ${resultList.length - successNumber} failed, ${resultList.length} in total.`);
            return resultList;
        });
}
